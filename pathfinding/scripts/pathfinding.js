...
pathFind.prototype.end = function () {
    ...
    if (this.drawOpen) { this.draw(this.openList, 'rgba(130,130,60,0.65)'); }
    if (this.drawClosed) { this.draw(this.closedList, 'rgba(255,0,0,0.5)'); }
    if (this.drawPath) { this.draw(this.pathList, 'rgba(0,180,0,1)'); }
...
pathFind.prototype.draw = function (list, color) {
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        if (this.showText) {
             ...
             var smallText = h * 0.33;
             var largeText = h * 0.36;
